...
//        path.setPatternInterval(10);
                ...
                Thread thread = new Thread(new Runnable() {
                    ...
                    public void run() {
                        try {
                            String url = "http://서버/schedule/" + scheduleId;
                            URL obj = new URL(url);
...
//        latLngList.add(new LatLng(SY, SX));
    ...
    public static void setWalking(double SX, double SY, double EX, double EY, List<LatLng> latLngList) {
        String urlInfo = null;
        try {
            urlInfo = "https://apis.openapi.sk.com/tmap/routes/pedestrian?version=1&startX=" + SX + "&startY=" + SY + "&endX=" + EX + "&endY=" + EY + "&startName=%EC%B6%9C%EB%B0%9C&endName=%EC%B6%9C%EB%B0%9C" + "&appKey=" + URLEncoder.encode("API_KEY", "UTF-8");
        } catch (Exception e) {
        }
        Log.d("zz", urlInfo);
        JsonObject responseJson = getJson(urlInfo);
